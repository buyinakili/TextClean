...
static std::string themedSvg(int id, COLORREF c) {
    ...
    if (id == IDR_CANCEL_SVG || id == IDR_COPY_SVG || id == IDR_SETTINGS_SVG || id == IDR_INFO_SVG || id == IDR_BILIBILI_SVG || id == IDR_GITHUB_SVG) { size_t path=s.find("<path"); size_t begin=path==std::string::npos?std::string::npos:s.find("d=\"",path); if (begin != std::string::npos) { begin += 3; size_t end=s.find('"', begin); std::string d=s.substr(begin, end-begin); char hex[8]; sprintf_s(hex, "#%02X%02X%02X", GetRValue(c), GetGValue(c), GetBValue(c)); return "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 24 24\"><path fill=\"" + std::string(id == IDR_SETTINGS_SVG ? "#000000" : hex) + "\" d=\"" + d + "\"/></svg>"; } }
    size_t doc=s.find("<!DOCTYPE"); if (doc != std::string::npos) { size_t end=s.find('>', doc); if (end != std::string::npos) s.erase(doc, end-doc+1); }
    if (id == IDR_ICON_SVG && s.find("viewBox=") == std::string::npos) { size_t tag=s.find("<svg"), end=s.find('>', tag); s.insert(end, " viewBox=\"0 0 1110 1024\""); }
...
bool Renderer::initialize(HINSTANCE instance) {
    ...
    impl_->logo=load(IDR_ICON_LARGE_PNG,impl_->logoStream); impl_->cancel=load(IDR_CANCEL_RASTER,impl_->cancelStream); impl_->copy=load(IDR_COPY_RASTER,impl_->copyStream); impl_->settings=load(IDR_SETTINGS_RASTER,impl_->settingsStream); impl_->payment=load(IDR_PAYMENT_PNG,impl_->paymentStream); return true;
}
void Renderer::shutdown() {
    if (!impl_) return; for (Cache* c : {&impl_->logoCache,&impl_->cancelCache,&impl_->copyCache,&impl_->settingsCache,&impl_->infoCache,&impl_->bilibiliCache,&impl_->githubCache}) if (c->bitmap) DeleteObject(c->bitmap);
    delete impl_->logo; delete impl_->cancel; delete impl_->copy; delete impl_->settings; delete impl_->payment; if(impl_->logoStream)impl_->logoStream->Release(); if(impl_->cancelStream)impl_->cancelStream->Release(); if(impl_->copyStream)impl_->copyStream->Release(); if(impl_->settingsStream)impl_->settingsStream->Release(); if(impl_->paymentStream)impl_->paymentStream->Release(); Gdiplus::GdiplusShutdown(impl_->gdiplusToken); delete impl_; impl_ = nullptr;
}
